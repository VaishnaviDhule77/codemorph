...
def test_structural_identical_calculator(calculator_analysis):
    module_b = analyze_source(
        (calculator_analysis.filename and __import__("pathlib").Path(
            __import__("tests.conftest", fromlist=["FIXTURES_DIR"]).FIXTURES_DIR
...
def test_data_flow_variable_rename():
    ...
    # params 1, defs 0, uses 1/3, flows 0, externals 1
    assert score == pytest.approx((1 + 0 + 1 / 3 + 0 + 1) / 5)
...
def test_equivalence_added_parameter_static():
    ...
    assert signal_by_name(report, "data_flow").score == pytest.approx(0.9)
    assert report.score_percent == 93
    assert report.label == "high"
...
def test_equivalence_smelly_static(smelly_source):
    ...
    # control-flow: bump collapses 4 statements to 2 -> (8 + 5/6) / 9
